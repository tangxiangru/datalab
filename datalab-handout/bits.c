 ...
 */
int bitCount(int x) {
    int flag1 = 0x11 | (0x11<<8);
    int flag2 = flag1 | (flag1<<16);
    int sum = x&flag2;
    sum = sum+((x>>1)&flag2);
    sum = sum+((x>>2)&flag2);
    sum = sum+((x>>3)&flag2);
...
int tmin(void) {
    ...
    int fitsBits(int x, int n) {
        ...
        int x= !(x^((x<<shiftNumber)>>shiftNumber));
        return x;
    }
    /*
    ...
    int divpwr2(int x, int n) {
        int x ;
        int flag1 = (((x>>31)&0x1)<<n) ;
        int flag2 = ~((x>>31)&0x1)+0x1 ;
        x = (x+flag1+flag2)>>n ;
